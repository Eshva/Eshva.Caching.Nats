Feature: Get entry from object-store based cache
  ...
  Background:
    Given entry with key 'existing' and value 'existing value' which expires in 3 minutes put into cache
    Given entry with key 'to be removed' and value 'to be removed value' which expires in 1 minutes put into cache
  ...
  Scenario: 06. Get cache entry operation triggers purging expired entries if its interval has passed
    ...
    Then 'to be removed' entry is not present in the object-store bucket
