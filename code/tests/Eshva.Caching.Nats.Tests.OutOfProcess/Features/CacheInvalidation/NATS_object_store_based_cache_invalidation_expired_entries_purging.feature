Feature: NATS object-store based cache invalidation expired entries purging
  ...
  Background:
    ...
    And NATS object-store based cache invalidation with purging interval 2 minutes with synchronous purge

  Scenario: 01. Cache invalidation should remove expired entries from object-store if passed more time than purging interval
  ...
  Scenario: 02. Cache invalidation should not remove any entries from object-store if passed less time than purging interval
